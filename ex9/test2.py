...
def test_add_car():
    ...
    # ensure none of the cars that should've failed to be added, were added
    # somehow

    # so we should only have have 7 occupied cells(5 from 'R', 2 from 'ZO').

    assert 7 == sum(
        1 for coord in board.cell_list() if board.cell_content(coord) if
        coord is not None)
...
def test_board_str_different_representations():
    ...
    # new one generates a DIFFERENT string (even though we don't know how
    # said string looks like)
...
def test_possible_moves_works():
    ...
    def car_moves(car_name):
        return sorted(move for name, move, _desc in board.possible_moves() if
                      car_name == name)
...
def test_move_car():
    ...
    def get_car_cords(car_name):
        return sorted(coord for coord in board.cell_list() if
                      board.cell_content(coord) == car_name)
