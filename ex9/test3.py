...
class Helper:
    ...
    def _run_game_process(self, json_file, input_txt):
        ...
        # check for other errors(e.g compilation errors, type errors) that
        # aren't
        ...
        if len(err) > 0 and "EOF" not in err:
            raise Exception(
                f"There was an unexpected error while running this test:\n"
    ...
    def finishes_with_exact_moves(self, car_cfg, moves):
        ...
        assert len(
            err) == 0, "The game should've terminated successfully after " \
                       "being given " \
 \
        "all valid moves, but instead it expected for more input."

    # Since the process library doesn't know if all standard input has been
    # consumed/processed, I make another

    # test to ensure that we don't win when given less than the needed moves
    #  to win.
    ...
    assert "EOF" in err, "When providing less than the exact moves for " \
                         "victory, the game should've " \
 \
    "errored(as it should expect for more input), but it has terminated " \
                         "successfully " \
 \
    "as if we have won."
...
def fails_with_given_moves(self, car_cfg, moves):
    ...
    assert "EOF" in err, "The game has terminated successfully despite not " \
                         "giving " \
 \
...
def __find_game_py():
    ...
    raise ValueError(f"Couldn't find game.py beginning at {cur_dir}\n."
                     ...
                     f"Are you sure you placed the tests within the "
                     f"exercise9\n"
...
def test_valid_simple():
    ...
    test_helper.finishes_with_exact_moves(cfg_file, ["O,u"] + [
        "Invalid,Cowabunga"] * 20 + ["O,u"] * 1337 + ["R,r"] * 6)

    test_helper.finishes_with_exact_moves(cfg_file, ["O,u"] + [
        "Invalid command"] * 20 + ["R,r"] * 6)
